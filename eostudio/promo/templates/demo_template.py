    ...
    def to_manim_script(self) -> str:
        ...
        for i, scene in enumerate(self.scenes):
            ...
            if i == 0:
                ...
                lines.append(
                    "        self.play(FadeOut(title), FadeOut(subtitle) if 'subtitle' in dir() else Wait(0))"
                )
            else:
        ...
        lines.extend(
            [
                "        # --- Final CTA ---",
                '        cta = Text("Try it now", font_size=56, color=WHITE)',
                "        cta.set_weight(BOLD)",
...
def create_quick_demo(
    product_name: str, features: list[str], url: str = ""
) -> DemoTemplate:
