...
class GameEngineExporter:
    ...
    def generate(self, screens: List[Dict[str, Any]], app_name: str = "MyGame") -> Dict[str, str]:
        if not screens:
            screens = [{"name": "MainScene", "components": []}]
        dispatch = {"godot": self._gen_godot, "unity": self._gen_unity, "unreal": self._gen_unreal}
        return dispatch[self._target](screens, app_name)

    # -- Godot --------------------------------------------------------

    def _gen_godot(self, screens: List[Dict[str, Any]], app_name: str) -> Dict[str, str]:
        files: Dict[str, str] = {}
        files["project.godot"] = (
        ...
        for s in screens:
            ...
            files[f"scenes/{name}.tscn"] = f'[gd_scene format=3]\n\n[node name="{name}" type="Node2D"]\n{nodes}'
    ...
    def _godot_nodes(self, components: List[Dict[str, Any]], parent: str) -> str:
        lines: List[str] = []
        type_map = {
            "Button": "Button",
            "Text": "Label",
    ...
    def _gen_unity(self, screens: List[Dict[str, Any]], app_name: str) -> Dict[str, str]:
        files: Dict[str, str] = {}
        for s in screens:
            name = self._pascal(s.get("name", "Main"))
            comps = s.get("components", [])
    ...
    def _unity_gameobjects(self, components: List[Dict[str, Any]], indent: int) -> str:
        lines: List[str] = []
        pad = "    " * indent
        for i, c in enumerate(components):
            ct = c.get("type", "Container")
    ...
    def _gen_unreal(self, screens: List[Dict[str, Any]], app_name: str) -> Dict[str, str]:
        files: Dict[str, str] = {}
        for s in screens:
            name = self._pascal(s.get("name", "Main"))
            comps = s.get("components", [])
    ...
    def _unreal_properties(self, components: List[Dict[str, Any]], indent: int) -> str:
        lines: List[str] = []
        pad = "    " * indent
        for i, c in enumerate(components):
            ct = c.get("type", "Container")
            ...
            if ct in ("Text", "Input"):
                lines.append(f'\n{pad}UPROPERTY(EditAnywhere)\n{pad}FString {prop_name} = TEXT("{lb}");\n')
            elif ct == "Button":
                lines.append(f"\n{pad}UPROPERTY(EditAnywhere)\n{pad}bool b{prop_name}Pressed = false;\n")
        return "".join(lines)
