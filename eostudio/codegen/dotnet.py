...
class DotNetAppGenerator:
    ...
    def generate(self, screens: List[Dict[str, Any]], app_name: str = "EoStudioApp") -> Dict[str, str]:
        ...
        dispatch = {"maui": self._gen_maui, "wpf": self._gen_wpf, "winui": self._gen_winui}
        return dispatch[self._target](screens, app_name)

    # -- MAUI ---------------------------------------------------------

    def _gen_maui(self, screens: List[Dict[str, Any]], app_name: str) -> Dict[str, str]:
        files: Dict[str, str] = {}
    ...
    def _maui_xaml_components(self, components: List[Dict[str, Any]], indent: int) -> str:
        lines: List[str] = []
        pad = "    " * indent
        for c in components:
            ...
            if ct == "Button":
                ...
                lines.append(
                    f'{pad}<Frame Padding="12" CornerRadius="8">\n'
    ...
    def _maui_shell_xaml(self, screens: List[Dict[str, Any]], app_name: str) -> str:
        tabs = ""
        for s in screens:
            name = self._pascal(s.get("name", "Home"))
            tabs += (
                f'        <ShellContent Title="{name}"\n'
    ...
    def _gen_wpf(self, screens: List[Dict[str, Any]], app_name: str) -> Dict[str, str]:
        files: Dict[str, str] = {}
    ...
    def _wpf_xaml_components(self, components: List[Dict[str, Any]], indent: int) -> str:
        lines: List[str] = []
        pad = "    " * indent
        for c in components:
            ct = c.get("type", "Container")
            ...
            if ct == "Button":
                ...
                lines.append(
                    f'{pad}<GroupBox Header="{lb}" Padding="8" Margin="0,4">\n'
    ...
    def _gen_winui(self, screens: List[Dict[str, Any]], app_name: str) -> Dict[str, str]:
        files: Dict[str, str] = {}
        files[f"{app_name}.csproj"] = (
            '<Project Sdk="Microsoft.NET.Sdk">\n'
            "  <PropertyGroup>\n"
