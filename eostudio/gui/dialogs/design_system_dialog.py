...
class DesignSystemDialog(tk.Toplevel):
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(
            header, text=f"Design System: {self._ds.name}", bg="#181825", fg=self._fg, font=("Segoe UI", 14, "bold")
    ...
    def _build_colors_tab(self, parent: tk.Frame) -> None:
        ...
        for token in self._ds.current_theme.by_category("color"):
            ...
            canvas.create_text(
                60, y + 15, text=f"{name}: {token.value}", fill=self._fg, font=("Consolas", 10), anchor=tk.W
            )
            y += 36
    ...
    def _build_typography_tab(self, parent: tk.Frame) -> None:
        for token in self._ds.current_theme.by_category("typography"):
            ...
            tk.Label(frame, text=name.upper(), bg="#313244", fg="#89b4fa", font=("Segoe UI", 8, "bold")).pack(
                anchor=tk.W
            )
            tk.Label(
                frame,
                text=f"{token.font_family} / {token.font_size}px / "
                f"Weight {token.font_weight} / Line {token.line_height}",
                bg="#313244",
                fg=self._fg,
                font=("Segoe UI", int(min(token.font_size * 0.6, 18))),
            ).pack(anchor=tk.W)
    ...
    def _build_components_tab(self, parent: tk.Frame) -> None:
        for comp_type, vs in self._ds.component_variants.items():
            comp_frame = tk.LabelFrame(
                parent, text=comp_type, bg=self._bg, fg="#cba6f7", font=("Segoe UI", 10, "bold"), bd=1, relief=tk.GROOVE
            )
            ...
            for variant in vs.variants:
                ...
                tk.Label(row, text=f"States: {states}", bg=self._bg, fg="#6c7086", font=("Segoe UI", 8)).pack(
                    side=tk.LEFT, padx=4
                )

    def _build_export_tab(self, parent: tk.Frame) -> None:
        tk.Label(parent, text="Export Design System", bg=self._bg, fg=self._fg, font=("Segoe UI", 12, "bold")).pack(
            padx=8, pady=(12, 8)
        )
    ...
    def _build_preview_tab(self, parent: tk.Frame) -> None:
        text = tk.Text(parent, bg="#181825", fg=self._fg, font=("Consolas", 9), wrap=tk.NONE, relief=tk.FLAT)
