...
class PromoEditor(tk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(left, text="Promo Templates", bg=self._bg, fg=self._fg, font=("Segoe UI", 11, "bold")).pack(
        ...
        for category in template_categories():
            ...
            for tmpl in list_templates(category):
                btn = tk.Button(
                    cat_frame,
                    text=tmpl.name.replace("_", " ").title(),
                    bg="#313244",
                    fg="#89b4fa",
                    relief=tk.FLAT,
                    font=("Segoe UI", 8),
        ...
        settings_frame = tk.LabelFrame(
            left, text="Settings", bg=self._bg, fg="#a6e3a1", font=("Segoe UI", 8, "bold"), bd=1, relief=tk.GROOVE
        )
        ...
        for label, var_name, default in [
            ...
            tk.Label(settings_frame, text=label, bg=self._bg, fg=self._fg, font=("Segoe UI", 8)).pack(
                fill=tk.X, padx=4, pady=(2, 0)
            )
            var = tk.StringVar(value=default)
            setattr(self, var_name, var)
            tk.Entry(settings_frame, textvariable=var, bg="#313244", fg=self._fg, font=("Segoe UI", 8)).pack(
        ...
        tk.Label(right, text="Export", bg=self._bg, fg=self._fg, font=("Segoe UI", 11, "bold")).pack(
            fill=tk.X, padx=8, pady=(8, 4)
        )
        ...
        tk.Label(format_frame, text="Format:", bg=self._bg, fg=self._fg, font=("Segoe UI", 8)).pack(anchor=tk.W, padx=4)
        ...
        tk.Label(preset_frame, text="Size Preset:", bg=self._bg, fg=self._fg, font=("Segoe UI", 8)).pack(
            anchor=tk.W, padx=4
        )
        ...
        tk.Button(
            right,
            text="Generate ffmpeg Command",
            bg="#313244",
            fg="#89b4fa",
            font=("Segoe UI", 8),
            relief=tk.FLAT,
            command=self._show_ffmpeg_command,
        ).pack(fill=tk.X, padx=8, pady=2)

        # Layer list
        layers_frame = tk.LabelFrame(
            right, text="Layers", bg=self._bg, fg="#cba6f7", font=("Segoe UI", 8, "bold"), bd=1, relief=tk.GROOVE
        )
        layers_frame.pack(fill=tk.BOTH, expand=True, padx=4, pady=4)

        self._layers_list = tk.Listbox(
            layers_frame, bg="#181825", fg=self._fg, font=("Consolas", 8), selectbackground="#313244", relief=tk.FLAT
        )
        ...
        tk.Button(
            ...
            font=("Segoe UI", 7),
            relief=tk.FLAT,
            command=self._add_device_layer,
        ).pack(side=tk.LEFT, padx=2)
        ...
        self._time_slider = ttk.Scale(transport, from_=0, to=100, orient=tk.HORIZONTAL, command=self._on_scrub)
        self._time_slider.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=8)

        self._time_label = tk.Label(transport, text="0.0s / 0.0s", bg="#181825", fg="#f9e2af", font=("Consolas", 9))
        ...
        self._info_label = tk.Label(
            center, text="Select a template to get started", bg=self._bg, fg="#6c7086", font=("Segoe UI", 10)
        )
    ...
    def _load_template(self, template: PromoTemplate) -> None:
        ...
        self._info_label.config(text=f"{template.name} | {template.width}x{template.height} | {template.duration}s")
    ...
    def _update_layers_list(self) -> None:
        ...
        if self._compositor:
            for layer in self._compositor.layers:
                ...
                self._layers_list.insert(tk.END, f"{visibility} {layer.name} [{layer.layer_type.value}]")
    ...
    def _render_preview(self) -> None:
        ...
        self._preview.create_rectangle(
            ox, oy, ox + frame["width"] * scale, oy + frame["height"] * scale, fill="#1a1a2e", outline="#45475a"
        )
        ...
        for layer_data in frame["layers"]:
            ...
            if layer_data["type"] == "text":
                ...
                self._preview.create_rectangle(dx, dy, dx + dw, dy + dh, fill="#181825", outline="#45475a", width=2)
                self._preview.create_oval(
                    dx + dw / 2 - 5, dy + dh - 15, dx + dw / 2 + 5, dy + dh - 5, outline="#585b70"
                )

        self._time_label.config(text=f"{self._current_time:.1f}s / {self._compositor.duration:.1f}s")
    ...
    def _add_text_layer(self) -> None:
        if self._compositor:
            self._compositor.add_text_layer("New Text", x=self._compositor.width / 2, y=self._compositor.height / 2)
            self._update_layers_list()
            self._render_preview()

    def _add_device_layer(self) -> None:
        if self._compositor:
            self._compositor.add_device_mockup("iphone_14", x=self._compositor.width / 2, y=self._compositor.height / 2)
            self._update_layers_list()
            self._render_preview()
    ...
    def _export_video(self) -> None:
        ...
        fmt_map = {
            ...
            "png_sequence": ExportFormat.PNG_SEQUENCE,
        }
        config = ExportConfig(
    ...
    def _show_ffmpeg_command(self) -> None:
        if self._compositor:
            ...
            text = tk.Text(win, bg="#181825", fg=self._fg, font=("Consolas", 10), wrap=tk.WORD, relief=tk.FLAT)
